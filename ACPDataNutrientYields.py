...
class ACPDataNutrientYields:
    ...
    def __init__(self,acpnamelist:ACPTrajectoryNamelist.ACPNamelist,method:str='ACRE'):
        ...
        self._init_vars(acpnamelist=acpnamelist,method=method)
    ...
    def _read_data(self,acpnamelist:ACPTrajectoryNamelist.ACPNamelist,method:str='SWAT PDF'):
        if acpnamelist.vars.verbose: print('Reading data')
        if   method.upper().find('ACRE') != -1: self._read_data_ACRE(acpnamelist)
        elif method.upper().find('SWAT') != -1: self._read_data_SWAT(acpnamelist)
    ...
    def _read_data_SWAT(self,acpnamelist:ACPTrajectoryNamelist.ACPNamelist):
        """"""
    ...
    def _read_data_SWAT_outputhru(self,acpnamelist:ACPTrajectoryNamelist.ACPNamelist): 
    ...
    def _set_nutrient_yield_distributions_SWAT(self,acpnamelist:ACPTrajectoryNamelist.ACPNamelist):
    ...
    def _get_baseline_yields_swatpdf(self):
        ...
        dt_baseline_kgha_key_parm = {parm:self.nutrientdata.baseline_kgha_distributions[parm].rvs() for parm in self.swatoutvars.n_vars + self.swatoutvars.p_vars}
